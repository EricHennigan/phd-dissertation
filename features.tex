...
Given JavaScript stores all variables as properties on an object, the semantics for accessing variables in JavaScript is the same as for accessing properties of objects.
...
For example, in Listing~\ref{list:funcdef}, the \code{this} keyword does not refer to the activation record object, but to the object holding the method definition, the built-in global object.
...
However, the lookup chain also involves labeled prototype references, so \FlowCore\ incorporates this chain into the label of the resulting value.
...
Implementation of \FlowCore\ required modification of helper methods internal to the JavaScriptCore interpreter to join in each component in union of labels applied to the resulting value of the lookup.
...
Instead it only affects the placement of the function and consequently the path of access a programmer uses to retrieve the function for invocation.
...
However, because of its construction at runtime, the new item carries a label dependent on a later execution context, which necessarily differs from the bottom label.
The presence of this label affects all later uses of the overridden item within the program.
In this manner, \FlowCore\ tracks the influence that an unexpected override may have on the rest of a JavaScript program.
...
Dynamically typed languages such as JavaScript present difficulties for tracking information flow.
Dependence on runtime input, the automatic type coercion of values, and the ability to use first-class functions as a runtime dispatch, all conspire to increase the difficulty of static flow analysis.
Consequently, \FlowCore\ performs label propagation during program execution.
...
\FlowCore\ modifies the instruction generator of JavaScriptCore, so that it instruments security instructions at parse time.
This modification allows \FlowCore\ to prevent certain categories of information flow leaks (\autoref{sec:tracking-capabilities}) by using a control flow stack to track the security label of the program counter (\autoref{sec:control-flow-stack}) at runtime.
...
- begin with dup_cflabel
- end popping scope with popj_cflabel
