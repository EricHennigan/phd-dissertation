...
In the event that the property is absent, the lookup proceeds by recursively scanning each object in the prototype chain, following the internal \bracketdbl{Prototype} property.
...
The lookup path may follow the internal \bracketdbl{scope} chain for properties defined in an outer lexical scope.
...
Assuming that the \bracketdbl{ToString} method has not been overridden by the programmer, the result of the coercion carries the same label as the given \code{key}, otherwise it carries the label of the returned result from the overloaded method call.
...
In the event that an object does not itself have a value associated with the requested \code{key}, the lookup proceeds recursively through objects in the prototype chain following \bracketdbl{Prototype} references.
...
\newcommand{\hashbracketdbl}[2]{\textlangle \bracketdbl{#1} \textbar\ \code{#2}\textrangle}
\begin{figure}[h!]
  \centering
\begin{tikzpicture}[
  ...
  \matrix [draw=gray, matrix anchor=west, column sep=1.5cm] (object1) {
    & \node[cellpath] (proto1) {\hashbracketdbl{Prototype}{P}}; \\
    & \node[cell] {\vdots}; \\
  ...
  \matrix [draw=gray, matrix anchor=west, column sep=1.5cm] at (7cm,0cm) (object2) {
    & \node[cell] (proto2) {\hashbracketdbl{Prototype}{Q}}; \\
    & \node[cell] {\vdots}; \\
...
It simplifies implementation, omits a source of label creep, improves runtime performance by eliding label joins, and permits a prototype implementation in WebKit's JIT compiler (\autoref{ch:implementation-sec:jitflow}).
...
Consequently, \FlowCore\ undertakes only enough static control-flow analysis to instrument additional instructions (\autoref{ch:implementation}) that assist in performing label propagation during program execution.

Fortunately, JavaScript uses structured control flow constructs.
This language feature permits the use of a static analysis that calculates exactly at which point the information flow instructions introduced in \autoref{ch:implementation-sec:instructions} need insertion into a program's instruction stream.
...
%Additionally, the labeling applies to any variables accessed along the \bracketdbl{scope} chain, using the same procedures for variable access on the \bracketdbl{property} chain.
