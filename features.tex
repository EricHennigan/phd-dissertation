...
In the latter two examples of Listing~\ref{list:property-access-notation}, \FlowCore\ first retrieves \code{obj} from the current activation object.
...
Flagrant use of values taken from extend reference chains can be yet another source of label creep in a JavaScript program.
...
Just as \FlowCore\ labels variables, it also labels references to functions, supporting the first-class definitions offered by JavaScript.
...
When accessing a function in order to invoke a call, the label of function follows the same rules as provided earlier for the lookup of variables (Section~\ref{sec:variables}).

The practice of using closures for data encapsulation and variable hiding, does not affect the implementation of \FlowCore\.
Instead it only affects the placement of the function and consequently the path of access a programmer must use to retrieve the function for invocation.
