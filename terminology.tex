...
After the founding of information flow as a program analysis technique in the late 1970's by Denning and Denning~\cite{denning-cert}, the field lay relatively quiescent until recently.
...
The rapid creation of so many new systems~\cite{staged-javascript}\cite{webssari}\cite{empirical-study}\cite{static-typing}\cite{XSS-tainting} using information flow techniques to attack web security problems has led to a difficulty in comparing research results across implementations.
...
Recent work applying information flow techniques has overloaded terminology established by Denning and Denning~\cite{denning-cert}.
For example, when Jang~et~al.~\cite{empirical-study} describe the capabilities of their JavaScript rewriting system, they introduce the term ``indirect'' to describe a flow for which they found no previous research.
This ``new'' terminology unfortunately overloads an existing use of the term by Denning and Denning~\cite{denning-cert}, prompting the creation of this chapter.

This work bases itself on Denning and Denning's~\cite{denning-cert} original categorization of the types of flows which can occur in an executable program because this categorization has become standard in the field.
...
This chapter refines the two categories of information flow established by Denning and Denning~\cite{denning-cert}, explicit flows (Section~\ref{sec:explicit}) and implicit flows (Section~\ref{sec:implicit}).
...
Denning and Denning~\cite{denning-cert} establish both the definition of explicit information flows and the descriptors shown in Table~\ref{table:explicitterminology}.
...
This paper preserves the use of the term ``indirect'' as originally defined by Denning and Denning~\cite{denning-cert} and not as overloaded by Jang~et~al.~\cite{empirical-study}.
}
\end{description}
...
Although Denning and Denning~\cite{denning-cert} establish the term \term{implicit flow} they did not refine the category into these two descriptors.
...
Because the literature lacks a refined terminology for the two descriptors of implicit flow, Jang~et~al.~\cite{empirical-study} coin the term ``indirect flow'' to refer to this kind of flow.
}
